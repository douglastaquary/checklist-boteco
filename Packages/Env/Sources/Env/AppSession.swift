...
public final class AppSession: ObservableObject {
  ...
  public func verifyTwoFactor(
    ...
    do {
      ...
      // Challenge is consumed by the server after a successful verify; drop local pending
      // before completing login so a later SQLite/local failure forces a fresh password login.
      pendingDeviceVerification = nil
      try await completeRemoteLogin(result: result, password: password)
      return result
    } catch {
