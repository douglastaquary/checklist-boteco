...
public class DynamoInventoryRepository implements InventoryRepository {
    ...
    public void save(CountSession value){ ensureHydrated(); sessions.put(value.id,value); put(value); }
    public List<CountSession> list(){ ensureHydrated(); return sessions.values().stream().sorted(Comparator.comparing((CountSession value)->value.countDate).reversed()).toList(); }
    public void delete(String id){ ensureHydrated(); if(sessions.remove(id)==null) throw new IllegalArgumentException("Contagem não encontrada"); dynamo.deleteItem(DeleteItemRequest.builder().tableName(table).key(Map.of("pk",AttributeValue.fromS("INVENTORY#"+id))).build()); }
